...
from mcp_server.preview_state import (
    build_track_summary,
    inspect_timeline_render_state,
    primary_visible_video_track,
    timeline_duration,
)

if TYPE_CHECKING:
    from mcp_server.project_state import ProjectStore, Timeline


def _inspect_timeline_render_state(timeline: "Timeline", time: float) -> dict[str, Any]:
    return inspect_timeline_render_state(timeline, time)


def _timeline_duration(timeline: "Timeline") -> float:
    return timeline_duration(timeline)


def _build_track_summary(timeline: "Timeline") -> list[dict[str, Any]]:
    return build_track_summary(timeline)


def _primary_visible_video_track(summary: list[dict[str, Any]]) -> dict[str, Any] | None:
    return primary_visible_video_track(summary)
